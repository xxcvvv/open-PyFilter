...
LastEditTime: 2024-04-05 15:34:06
Description: 
'''
import os
import sys
...
def main(Config: dict, **kwargs):
    ...
    X_f_dust_read = np.zeros([Nspec, Nlev, Nlat, Nlon,
    ...
    for i_ensem in range(Ne):
        X_f_aod[:, :,
                i_ensem] = (X_f_aod_read[:, :, i_ensem].reshape([1, Ns]) *
                            dust_ratio_layers)
    ...
    logging.info('Model localization filter rate : %.2f ' %
                 (local_rate * 100) + '%')

    # location space localization
    lon_meshed, lat_meshed = np.meshgrid(model_lon, model_lat)
    lon_space = np.array([lon_meshed
                          for i_lev in range(Nlev)]).reshape([Nlev, Ns])
    lat_space = np.array([lat_meshed
                          for i_lev in range(Nlev)]).reshape([Nlev, Ns])
    ...
    logging.debug('%s BC_PM10 in model space.' % (obs.values['bc_pm10'].size))
    ...
    #     logging.info('%s %s' % (np.mean(obs.values['modis_dod'][i_lev]),
    #                                np.mean(obs.error['modis_dod'][i_lev])))

    obs.reduce_dim(Ns, Nlev)
    logging.debug('%s MODIS DOD in model space.' %
    ...
    obs.reduce_dim(Ns, Nlev)
    logging.debug('%s VIIRS DOD in model space.' %
    ...
    logging.info('Mean of posterior : %s and Mean of prior : %s' %
